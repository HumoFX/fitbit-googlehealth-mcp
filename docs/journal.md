...
## 2026-05-31 / Google Health ローンチ後の状況確認

約 5 週間ぶりにプロジェクトを開いた。その間に**消費者向け Google Health アプリが正式ローンチ**(2026-05-07 発表、5/19〜5/26 ロールアウト)— Fitbit アプリの後継。API 仕様も動いている可能性があるので、まず最新状況を確定させてから手を動かすか決めることにした。

### やったこと

- **実機プローブ**: デプロイ済み Worker 経由で `get_profile` / `list_devices` を叩き、「いまこの瞬間 Fitbit Web API が生きているか」を一次確認。ユーザーのモバイルアプリに頼らず、自分でサーバーを叩いて確かめる方針(過去の food-log 調査と同じやり方)
- **多角 Web 調査**: 6 角度の fan-out 調査 → 重要主張を独立検証 → ギャップ補完(34/35 を confirmed)。差分は [`research.md`](research.md) 末尾「ローンチ後アップデート(2026-05-31)」にまとめた

### 効いた気づき

1. **Fitbit Web API はまだ生きている**。`get_profile` が正常応答、`Fitbit Air`(ローンチ同時発表の新トラッカー)が当日同期。9 月停止は不変なので **急ぐ必要はない**
2. **アカウント統合は Fitbit Web API トークンを無効化しない**。これが設計時の最大の懸念だったが空振り — merged アカウントの refresh token が 401 なしで今も refresh できることを実機で確認。「再同意必須」は GH API へ移行する時だけの話で、統合と混同していた(`research.md §1, §3` に訂正を注記)
3. **GH API の食事書き込みが解禁**(5/26)。設計時は「未リリース」前提で Fitbit 側 PFC preset ワークアラウンドを組んだが、移行後はこれが不要になる可能性
4. **Pattern B は移行後も生きる**が、Google OAuth 同意画面を "In production" に publish しないと refresh token が 7 日失効する罠がある(未 verified でも production にはできる)

### 判断

- **コード変更はしない**。9 月までは現状維持でよく、GH API はまだ breaking change が landing 中(5/26 にもスコープ分割)。"動く的" を今撃つのは損
