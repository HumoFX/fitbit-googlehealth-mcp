...
export function registerActivityReadTools(
  ...
  server.registerTool(
    ...
    async ({ beforeDate, limit }) => {
      try {
        const bd = beforeDate ?? (await resolveToday(env, provider));
